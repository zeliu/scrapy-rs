...
impl UrlFilterMiddleware {
    ...
    pub fn new(allowed_patterns: Vec<Regex>, denied_patterns: Vec<Regex>) -> Self {
        Self { allowed_patterns, denied_patterns }
    }
    ...
    pub fn from_strings(allowed_patterns: Vec<&str>, denied_patterns: Vec<&str>) -> Result<Self> {
        let allowed = allowed_patterns
            .into_iter()
            .map(|p| Regex::new(p).map_err(|e| Error::middleware(format!("Invalid regex: {}", e))))
...
impl RequestMiddleware for UrlFilterMiddleware {
    async fn process_request(&self, request: Request, _spider: &dyn Spider) -> Result<Request> {
        ...
        for pattern in &self.denied_patterns {
            if pattern.is_match(url_str) {
                return Err(Error::middleware(format!(
                    "URL {} matched denied pattern {}", url_str, pattern
                )));
            }
        }
        ...
        if !self.allowed_patterns.is_empty() {
            ...
            if !allowed {
                return Err(Error::middleware(format!(
                    "URL {} did not match any allowed pattern", url_str
                )));
            }
        }
...
impl ResponseMiddleware for RetryMiddleware {
    async fn process_response(&self, response: Response, _spider: &dyn Spider) -> Result<Response> {
        ...
        info!(
            ...
            request.url, response.status, retry_count + 1, self.max_retries
        );
        
        // Add the request back to the meta of the response
        let mut response = response;
        response.meta.insert("retry_request".to_string(), serde_json::to_value(request).unwrap());
...
impl ChainedRequestMiddleware {
    ...
    fn ensure_sorted(&mut self) {
        if !self.sorted {
            ...
            if log::log_enabled!(log::Level::Debug) {
                let middleware_names: Vec<String> = self.middlewares
                    .iter()
    ...
    pub fn get_middlewares(&mut self) -> &[MiddlewareWrapper<dyn RequestMiddleware>] {
        self.ensure_sorted();
        &self.middlewares
    }
}
...
impl RequestMiddleware for ChainedRequestMiddleware {
    async fn process_request(&self, mut request: Request, spider: &dyn Spider) -> Result<Request> {
        // Create a mutable copy of self to ensure sorting
        let mut this = self.clone();
        this.ensure_sorted();
        
        println!("ChainedRequestMiddleware::process_request - Middlewares count: {}", this.middlewares.len());
        for (i, wrapper) in this.middlewares.iter().enumerate() {
            println!("Processing middleware {}: {} (priority: {:?})", i, wrapper.name, wrapper.priority);
...
impl ChainedResponseMiddleware {
    ...
    /// Get the middlewares in priority order
    pub fn get_middlewares(&mut self) -> &[MiddlewareWrapper<dyn ResponseMiddleware>] {
        self.ensure_sorted();
        &self.middlewares
    }
}
...
impl ResponseMiddleware for ChainedResponseMiddleware {
    async fn process_response(&self, mut response: Response, spider: &dyn Spider) -> Result<Response> {
        // Create a mutable copy of self to ensure sorting
        let mut this = self.clone();
        this.ensure_sorted();
        
        for wrapper in &this.middlewares {
            if wrapper.middleware.should_process_response(&response, spider) {
                response = wrapper.middleware.process_response(response, spider).await?;
            }
        }
...
mod tests {
    ...
    async fn test_default_headers_middleware() {
        ...
        let processed = middleware.process_request(request, &TestSpider).await.unwrap();
    ...
    async fn test_response_logger_middleware() {
        ...
        let processed = middleware.process_response(response, &TestSpider).await.unwrap();
    ...
    async fn test_retry_middleware() {
        ...
        // First retry
        let processed = middleware.process_response(response.clone(), &TestSpider).await.unwrap();
        ...
        response_with_count.meta.insert("retry_count".to_string(), serde_json::json!(1));
        let processed = middleware.process_response(response_with_count, &TestSpider).await.unwrap();
        assert!(processed.meta.contains_key("retry_request"));
        
        // Third retry (with retry_count = 2)
        let mut response_with_count = response.clone();
        response_with_count.meta.insert("retry_count".to_string(), serde_json::json!(2));
        let processed = middleware.process_response(response_with_count, &TestSpider).await.unwrap();
        assert!(processed.meta.contains_key("retry_request"));
        
        // Fourth retry (with retry_count = 3) - should not retry
        let mut response_with_count = response.clone();
        response_with_count.meta.insert("retry_count".to_string(), serde_json::json!(3));
        let processed = middleware.process_response(response_with_count, &TestSpider).await.unwrap();
        assert!(!processed.meta.contains_key("retry_request"));
    }
    ...
    async fn test_middleware_chain_execution_order() {
        ...
        impl RequestMiddleware for TrackingMiddleware {
            async fn process_request(&self, request: scrapy_rs_core::request::Request, _spider: &dyn scrapy_rs_core::spider::Spider) -> scrapy_rs_core::error::Result<scrapy_rs_core::request::Request> {
            ...
            fn should_process_request(&self, _request: &scrapy_rs_core::request::Request, _spider: &dyn scrapy_rs_core::spider::Spider) -> bool {
                true
            }
        }
