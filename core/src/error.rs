...
impl Error {
    ...
    pub fn retry_delay(&self) -> Option<Duration> {
        match self {
            Self::Http {
                error: HttpError::RateLimited { retry_after, .. },
                ..
            } => *retry_after,
            Self::Network {
                error: NetworkError::Timeout(_),
                ..
...
impl From<std::io::Error> for Box<Error> {
    fn from(error: std::io::Error) -> Self {
        Box::new(Error::IoError(error.to_string()))
    }
}
...
impl From<serde_json::Error> for Box<Error> {
    fn from(error: serde_json::Error) -> Self {
        Box::new(Error::SerdeError(error.to_string()))
    }
}

// Implement From for url::ParseError
impl From<url::ParseError> for Box<Error> {
    fn from(error: url::ParseError) -> Self {
        Box::new(Error::UrlParseError(error))
    }
}
