...
impl Settings {
    ...
    pub fn from_toml(contents: &str) -> Result<Self> {
        let raw: HashMap<String, serde_json::Value> = toml::from_str(contents)
    ...
    pub fn from_yaml(contents: &str) -> Result<Self> {
        #[cfg(feature = "yaml")]
        {
            let raw: HashMap<String, serde_json::Value> = serde_yaml::from_str(contents)
        ...
        {
            Err(SettingsError::YamlParse("YAML support not enabled".to_string()))
        }
    }
        ...
        self.raw.get(key)
            ...
            .and_then(|value| {
                serde_json::from_value(value.clone())
                    .map_err(|e| SettingsError::JsonParse(e))
            })
    }
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        let contents = match format {
            SettingsFormat::Toml => toml::to_string(&self.raw)
                .map_err(|e| SettingsError::TomlParse(e.to_string()))?,
            SettingsFormat::Json => serde_json::to_string_pretty(&self.raw)
                .map_err(|e| SettingsError::JsonParse(e))?,
            SettingsFormat::Yaml => {
                #[cfg(feature = "yaml")]
                {
                ...
                #[cfg(not(feature = "yaml"))]
                {
                    return Err(SettingsError::YamlParse("YAML support not enabled".to_string()));
                }
            },
...
mod tests {
    ...
    fn test_from_toml() {
        ...
        assert_eq!(settings.get::<String>("user_agent").unwrap(), "scrapy_rs/0.1.0");
        assert_eq!(settings.get::<bool>("follow_redirects").unwrap(), true);
    }
