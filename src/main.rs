...
use scrapy_rs::engine::{Engine, EngineConfig};
use scrapy_rs_core::spider::BasicSpider;
use scrapy_rs::downloader::{HttpDownloader, DownloaderConfig};
use scrapy_rs::scheduler::MemoryScheduler;
...
fn start_project(name: &str, directory: Option<&str>) {
    ...
    println!("Creating project '{}' in directory '{}'", name, project_dir.display());
    ...
    let settings_content = format!(r#"# Scrapy-RS settings file
...
"#, name);
    ...
    if let Err(e) = fs::write(&init_path, "# This file is required to make Python treat the directory as a package\n") {
...
"#;
    ...
    println!("You can now cd into '{}' and start creating your spiders", project_dir.display());
}
...
fn gen_spider(name: &str, domain: &str, template: &str) {
    ...
    let spider_content = match template {
        "basic" => {
            format!(r#"from scrapy-rs import PySpider, PyItem, PyRequest
...
fn run_spider_file(spider_file: &Path, output: Option<&str>, format: &str, settings_path: Option<&str>) {
    // Read the spider file
    let spider_code = match fs::read_to_string(spider_file) {
        Ok(code) => code,
        Err(e) => {
    ...
    if let Ok(concurrent_requests) = settings.get::<usize>("CONCURRENT_REQUESTS") {
        println!("Using concurrent requests from settings: {}", concurrent_requests);
    ...
    println!("Creating engine with scheduler type: {:?}, strategy: {:?}", 
