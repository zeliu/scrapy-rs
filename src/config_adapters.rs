...
use scrapy_rs_engine::{EngineConfig, SchedulerType, ResourceLimits};
use scrapy_rs_scheduler::{MemoryScheduler, CrawlStrategy};
use crate::settings::{Settings, Result, SettingsError};
use std::sync::Arc;
...
pub fn engine_config_from_settings(settings: &Settings) -> Result<EngineConfig> {
    ...
    let scheduler_type = match settings.get::<String>("SCHEDULER_TYPE").unwrap_or_default().as_str() {
    ...
    let crawl_strategy = match settings.get::<String>("CRAWL_STRATEGY").unwrap_or_default().as_str() {
...
pub fn create_spider_from_settings(settings: &Settings) -> Result<Arc<BasicSpider>> {
    let name: String = settings.get("BOT_NAME").unwrap_or_else(|_| "scrapy_rs".to_string());
    ...
    // Add custom settings
    if let Ok(custom_settings) = settings.get::<serde_json::Map<String, serde_json::Value>>("SPIDER_SETTINGS") {
...
pub fn create_scheduler_from_settings(settings: &Settings) -> Result<Arc<dyn scrapy_rs_scheduler::Scheduler>> {
    // Get scheduler type from settings
    let scheduler_type = match settings.get::<String>("SCHEDULER_TYPE").unwrap_or_default().as_str() {
        "domain_group" => SchedulerType::DomainGroup,
        "breadth_first" => SchedulerType::BreadthFirst,
    ...
    match scheduler_type {
        ...
        SchedulerType::DomainGroup => {
            ...
            Ok(Arc::new(scrapy_rs_scheduler::DomainGroupScheduler::new(config)))
        }
        SchedulerType::BreadthFirst => {
            let max_depth = settings.get("MAX_DEPTH").ok();
            Ok(Arc::new(scrapy_rs_scheduler::BreadthFirstScheduler::new(max_depth)))
        }
        SchedulerType::DepthFirst => {
            let max_depth = settings.get("MAX_DEPTH").ok();
            Ok(Arc::new(scrapy_rs_scheduler::DepthFirstScheduler::new(max_depth)))
        }
    }
} 