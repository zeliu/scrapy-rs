...
use scrapy_rs::config_adapters::{
    ...
    downloader_config_from_settings,
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let bot_name: String = settings.get("BOT_NAME")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    let user_agent: String = settings.get("USER_AGENT")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    let concurrent_requests: usize = settings.get("CONCURRENT_REQUESTS")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    let download_delay_ms: u64 = settings.get("DOWNLOAD_DELAY_MS")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    let follow_redirects: bool = settings.get("FOLLOW_REDIRECTS")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    let start_urls: Vec<String> = settings.get("START_URLS")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    let allowed_domains: Vec<String> = settings.get("ALLOWED_DOMAINS")
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    ...
    let engine_config = engine_config_from_settings(&settings)
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    println!("  Created EngineConfig: concurrent_requests={}", engine_config.concurrent_requests);
    
    // Create DownloaderConfig
    let downloader_config = downloader_config_from_settings(&settings)
        .map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    println!("  Created DownloaderConfig: timeout={}s", downloader_config.timeout);
