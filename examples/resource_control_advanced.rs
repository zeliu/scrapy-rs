...
impl MemoryIntensiveSpider {
    ...
    fn allocate_memory(&mut self, size_mb: usize) {
        ...
        println!(
            "Allocated {}MB of memory, total blocks: {}",
            size_mb,
...
async fn run_with_resource_control() -> Result<()> {
    ...
    let config = EngineConfig {
        ...
        concurrent_requests: 5, // Try to use 5 concurrent requests, but resource control will limit to 2
        ..Default::default()
    };
...
async fn run_without_resource_control() -> Result<()> {
    ...
    let config = EngineConfig {
        ...
        concurrent_requests: 5, // Use 5 concurrent requests
        ..Default::default()
    };
