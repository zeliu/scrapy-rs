...
use scrapy_rs_core::error_handler::{ErrorAction, ErrorManager, ErrorHandler};
...
use scrapy_rs_scheduler::{
    BreadthFirstScheduler, CrawlStrategy, DomainGroupScheduler, MemoryScheduler, Scheduler,
    SchedulerConfig, DepthFirstScheduler,
};
...
impl Engine {
    ...
    fn create_scheduler(config: &EngineConfig) -> Result<Arc<dyn Scheduler>> {
        match config.scheduler_type {
            SchedulerType::Memory => {
                Ok(Arc::new(MemoryScheduler::new()))
            }
            ...
            SchedulerType::DepthFirst => {
                Ok(Arc::new(DepthFirstScheduler::new(config.max_depth)))
            }
        }
    }
    ...
    pub async fn run(&mut self) -> Result<EngineStats> {
        ...
        *running = true;
        ...
        self.signals.connect(Signal::EnginePaused, move |_| {
        ...
        let running_clone = self.running.clone();
        self.signals.connect(Signal::EngineResumed, move |_| {
            let running = running_clone.clone();
            tokio::spawn(async move {
                let mut r = running.write().await;
                *r = true;
        ...
        self.signals.send_catch_log(Signal::EngineStarted, SignalArgs::None).await;
        
        // Open the spider
        info!("Opening spider: {}", self.spider.name());
        self.request_middlewares.spider_opened(&*self.spider).await?;
        self.response_middlewares.spider_opened(&*self.spider).await?;
        ...
        for request_result in start_requests {
            match request_result {
                ...
                Err(e) => {
                    ...
                    let error_with_context = Error::other(format!("Error creating start request: {}", e))
                        .with_component("spider_start_requests")
                        .with_spider_name(self.spider.name());
                    
                    match self.error_manager.handle_error(&error_with_context, &*self.spider).await {
                        Ok(ErrorAction::Retry { request, delay, reason }) => {
                ...
                while let Some(item) = item_rx.recv().await {
                    ...
                    match pipelines.process_dynamic_item(item.clone(), &*spider).await {
                        ...
                        Err(e) => {
                            ...
                            let error_with_context = Error::other(format!("Error processing item: {}", e))
                                .with_component("pipeline")
                                .with_spider_name(spider.name());
                            
                            match error_manager.handle_error(&error_with_context, &*spider).await {
                                Ok(ErrorAction::Retry { request: _, delay: _, reason }) => {
                                ...
                                Ok(ErrorAction::Ignore) => {
                                    debug!("Ignoring item processing error: {}", error_with_context);
                                },
                                Err(e) => {
        ...
        'main_loop: while !self.scheduler.is_empty().await || active_tasks > 0 || !tasks.is_empty() {
            ...
            while active_tasks < self.config.concurrent_requests && !self.scheduler.is_empty().await {
                ...
                if let Some(request) = self.scheduler.next().await {
                    ...
                    let processed_request = match self.request_middlewares.process_request(request, &*self.spider).await {
                        Ok(req) => req,
                        Err(e) => {
                            // Handle error from request middleware
                            let error_with_context = Error::other(format!("Request middleware rejected request: {}", e))
                            ...
                            self.signals.send_catch_log(
                                Signal::ErrorOccurred,
                                SignalArgs::Error(format!("Request middleware rejected request: {}", e)),
                            ).await;
                    ...
                    self.signals.send_catch_log(
                        Signal::RequestSent,
                        SignalArgs::Request(processed_request.clone()),
                    ).await;
                    ...
                    let task = tokio::spawn(async move {
                        ...
                        let response = match downloader.download(processed_request.clone()).await {
                            ...
                            Err(e) => {
                                ...
                                let error_with_context = Error::other(format!("Error downloading request: {}", e))
                                ...
                                signals.send_catch_log(
                                    Signal::ErrorOccurred,
                                    SignalArgs::Error(format!("Error downloading request: {}", e)),
                                ).await;
                        ...
                        signals.send_catch_log(
                            ...
                            SignalArgs::Response(response.clone()),
                        ).await;
                        
                        // Process the response through middleware
                        let processed_response = match response_middlewares.process_response(response, &*spider).await {
                            Ok(resp) => resp,
                            Err(e) => {
                                // Handle error from response middleware
                                let error_with_context = Error::other(format!("Response middleware rejected response: {}", e))
                                ...
                                signals.send_catch_log(
                                    Signal::ErrorOccurred,
                                    SignalArgs::Error(format!("Response middleware rejected response: {}", e)),
                                ).await;
                        ...
                        if let Some(retry_request) = processed_response.meta.get("retry_request") {
                            if let Ok(request) = serde_json::from_value::<Request>(retry_request.clone()) {
                                ...
                                if let Err(e) = scheduler.enqueue(request).await {
                                    ...
                                    signals.send_catch_log(
                                        Signal::ErrorOccurred,
                                        SignalArgs::Error(format!("Error enqueueing retry request: {}", e)),
                                    ).await;
                                }
                                return;
                            }
                        ...
                        let parse_output = match spider.parse(processed_response.clone()).await {
                            ...
                            Err(e) => {
                                ...
                                let error_with_context = Error::other(format!("Error parsing response: {}", e))
                        ...
                        for request in parse_output.requests {
                            ...
                            if let Err(e) = scheduler.enqueue(request).await {
                                ...
                                signals.send_catch_log(
                                    Signal::ErrorOccurred,
                                    SignalArgs::Error(format!("Error enqueueing request: {}", e)),
                                ).await;
                            }
                        }
                        ...
                        for item in parse_output.items {
                            ...
                            if let Err(e) = item_tx.send(item).await {
                                ...
                                signals.send_catch_log(
                                    Signal::ErrorOccurred,
                                    SignalArgs::Error(format!("Error sending item to pipeline: {}", e)),
                                ).await;
                            }
                        }
                    });
        ...
        item_processor_task.await.map_err(|e| Error::Other {
        ...
        self.response_middlewares.spider_closed(&*self.spider).await?;
        self.request_middlewares.spider_closed(&*self.spider).await?;
        ...
        if let Some(task) = stats_task {
            ...
            *running = false;
            ...
            task.await.map_err(|e| Error::Other {
        ...
        info!("Error stats: {} total errors, {} retries, {} aborts, {} skips, {} ignored",
        ...
        signals_clone.send_catch_log(Signal::EngineStopped, SignalArgs::None).await;
    ...
    pub async fn load_state(&self, path: &str) -> Result<()> {
        ...
        // Deserialize state
        let mut state: EngineState = serde_json::from_str(&serialized)
            .map_err(|e| Error::Other {
    ...
    impl Spider for TestSpider {
        ...
        async fn parse(&self, response: Response) -> Result<ParseOutput> {
            ...
            if response.url.path() == "/page1" {
                if let Ok(request) = Request::get(format!("{}/page2", response.url.origin().ascii_serialization())) {
