...
use scrapy_rs_core::item::{Item, DynamicItem};
...
pub trait Pipeline: Send + Sync + 'static {
    ...
    async fn process_dynamic_item(&self, item: DynamicItem, spider: &dyn Spider) -> Result<DynamicItem>;
...
impl Pipeline for DummyPipeline {
    async fn process_dynamic_item(&self, item: DynamicItem, _spider: &dyn Spider) -> Result<DynamicItem> {
        Ok(item)
    }
}
...
impl Pipeline for JsonFilePipeline {
    ...
    async fn process_dynamic_item(&self, item: DynamicItem, _spider: &dyn Spider) -> Result<DynamicItem> {
        ...
        if let Some(file) = file_guard.as_mut() {
            ...
            let json = serde_json::to_string_pretty(&item).map_err(|e| Error::SerdeError(e.to_string()))?;
            
            // Write the item to the file
            file.write_all(json.as_bytes()).map_err(|e| Error::IoError(e.to_string()))?;
            file.write_all(b",\n").map_err(|e| Error::IoError(e.to_string()))?;
    ...
    async fn close_spider(&self, _spider: &dyn Spider) -> Result<()> {
        ...
        if let Some(mut file) = file_guard.take() {
            ...
            let pos = file.metadata().map_err(|e| Error::IoError(e.to_string()))?.len();
            if pos > 2 {
                use std::io::Seek;
                file.seek(std::io::SeekFrom::End(-2)).map_err(|e| Error::IoError(e.to_string()))?;
            }
    ...
    F: Fn(&DynamicItem) -> bool + Send + Sync + 'static,
{
    async fn process_dynamic_item(&self, item: DynamicItem, _spider: &dyn Spider) -> Result<DynamicItem> {
        if (self.filter)(&item) {
            ...
            Err(Error::item("Item filtered out"))
        }
    }
}
...
pub fn to_dynamic_item<I: Item>(item: &I) -> Result<DynamicItem> {
    ...
    if let serde_json::Value::Object(map) = json_value {
        ...
        Err(Error::item("Failed to convert item to DynamicItem"))
    }
}
...
mod tests {
    ...
    impl Spider for TestSpider {
        ...
        async fn parse(&self, _response: scrapy_rs_core::response::Response) -> Result<scrapy_rs_core::spider::ParseOutput> {
            unimplemented!()
        }
    }
    ...
    async fn test_dummy_pipeline() {
        ...
        let spider = TestSpider { name: "test".to_string() };
        
        let mut item = DynamicItem::new("test");
        item.set("field1", "value1");
        
        let result = pipeline.process_dynamic_item(item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("field1").unwrap().as_str().unwrap(), "value1");
    }
    
    #[tokio::test]
    async fn test_filter_pipeline() {
        let allow_pipeline = FilterPipeline::new(|item: &DynamicItem| {
            item.has_field("keep")
        });
        
        let deny_pipeline = FilterPipeline::new(|item: &DynamicItem| {
            !item.has_field("drop")
        });
        ...
        // Test allow pipeline
        let result = allow_pipeline.process_dynamic_item(keep_item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("keep").unwrap().as_bool().unwrap(), true);
        
        let result = allow_pipeline.process_dynamic_item(drop_item.clone(), &spider).await;
        assert!(result.is_err());
        
        // Test deny pipeline
        let result = deny_pipeline.process_dynamic_item(keep_item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("keep").unwrap().as_bool().unwrap(), true);
        
        let result = deny_pipeline.process_dynamic_item(drop_item.clone(), &spider).await;
        assert!(result.is_err());
    }
    ...
    async fn test_pipeline_enum() {
        ...
        let result = dummy.process_dynamic_item(item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("test").unwrap().as_str().unwrap(), "value");
        
        let result = log.process_dynamic_item(item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("test").unwrap().as_str().unwrap(), "value");
        
        let result = filter.process_dynamic_item(item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("test").unwrap().as_str().unwrap(), "value");
        
        let chained = PipelineType::Chained(vec![dummy, log, filter]);
        let result = chained.process_dynamic_item(item.clone(), &spider).await.unwrap();
        assert_eq!(result.get("test").unwrap().as_str().unwrap(), "value");
    }
} 